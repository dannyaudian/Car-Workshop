...
class WorkOrderBillingServicePackage(Document):
    """
    Child DocType for Service Package items in Work Order Billing.
    Represents a billable service package with quantity and rate.
    """
    
    def validate(self) -> None:
        """Validate required fields and values."""
        if not self.service_package:
            frappe.throw(_("Service Package is required"))
    ...
    def get_default_rate(self, price_list: Optional[str] = None, date: Optional[str] = None) -> float:
        """
        Get the default rate for the service package from Item Price or Service Price List.
        ...
        # Get the item linked to the service package
        item_code = frappe.db.get_value("Service Package", self.service_package, "item")
        if not item_code:
            return 0
        ...
        # Last resort, return item standard rate or service package rate
        return (
            frappe.db.get_value("Item", item_code, "standard_rate") or
            frappe.db.get_value("Service Package", self.service_package, "rate") or
            0
        )